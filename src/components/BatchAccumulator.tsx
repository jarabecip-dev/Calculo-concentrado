...
import { Layers, Trash2, Plus, ArrowDownToLine, Droplets } from 'lucide-react';
        ...
        {batchItems.length === 0 ? (
          <div className="text-center py-6 px-4 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-slate-500">
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Utilice el botón "+ Acumular a Lote" en la calculadora para sumar varios tamaños a una misma orden.
            </p>
          </div>
        ) : (
            ...
            <div className="p-4 rounded-xl bg-amber-50 border border-amber-300/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <span className="text-xs font-semibold text-amber-900 block">
                  Total Lote ({batchItems.length} formatos / {formatNumber(totalBatchBottles, 0)} botellas)
                </span>
